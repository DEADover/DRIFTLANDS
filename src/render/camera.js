...
export class ChaseCamera {
  ...
  update(dt, car, opts = {}) {
    ...
    // straight or a tight corner can never push the car off the edge. The
    // client saw exactly that: "in some parts of the map the camera cannot keep
    // up and loses the car".
    const vFov = (this.camera.fov * Math.PI) / 180;
    const halfFrame = this.distance * Math.tan(vFov / 2) * this.camera.aspect;
    const maxLead = Math.max(8, halfFrame * 0.45);
    const leadDist = Math.min(speed * lead, maxLead);
